...
public class PlayerInput : MonoBehaviour
{
    ...
    private void Update() {
        // Completely ignore board input while the promotion menu is open -
        // stops any click from being misinterpreted as a board move while
        // waiting for the player to pick a piece.
        if (uiManager != null && uiManager.PromotionPending) return;

        // AI vs AI: no human plays either side, so never accept board input.
        if (gameManager.gameMode == GameManager.GameMode.AIvsAI) return;
            ...
            foreach (Move move in moves){
                if (move.to.rank == coord.rank && move.to.file == coord.file){
                    ...
                    if (board.CanPromote(coord)){
                        // Refresh the board immediately so the pawn visually
                        // sits on the back rank right away, instead of the
                        // board looking "stale" until a piece is picked.
                        boardUI.UpdateBoard(board, gameManager.boardFlipped);
                        boardUI.ResetAllSquareColors();
